...
def print_menu_line(direction, leads_to):
    ...
    """
    # >>> print_menu_line("south", "MJ and Simon's room")
    # output: Go SOUTH to MJ and Simon's room.
...
def print_menu(exits):
    ...
    for _exit in exits:
        print_menu_line(_exit, exit_leads_to(exits, _exit))
        #print('Go ' + _exit.upper() + ' to ' + exit_leads_to(exits, _exit))
...
def menu(exits):
    ...
    while True:
        ...
        _str = input()
        # Normalise the input
        _str = normalise_input(_str)
        # Check if the input makes sense (is valid exit)
        if is_valid_exit(exits, _str):
            return _str
        else:
            print('Invalid exit')
...
def move(exits, direction):
    ...
    """
    # get the room name
    location = exit_leads_to(exits, direction)

    # return location from the dict
    if location.find('Simon') != -1:
        return rooms['Admins']
    elif location.find('tutor') != -1:
        return rooms['Tutor']
    elif location.find('parking lot') != -1:
        return rooms['Parking']
    elif location.find('the general') != -1:
        return rooms['Office']
    else:
        return rooms['Reception']
